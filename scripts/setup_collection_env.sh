...
# Data collection + processing environment (without data selection UI).

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd -- "${SCRIPT_DIR}/.." && pwd)"
ENV_DIR="${COLLECTION_ENV_DIR:-${PROJECT_ROOT}/.venv-collection}"
...
usage() {
    cat <<'EOF'
Usage: bash scripts/setup_collection_env.sh [options]

Options:
  --env-dir PATH          Virtual environment path (default: .venv-collection)
...
Environment variables:
  COLLECTION_ENV_DIR     Same as --env-dir
...
echo "Creating collection environment: ${ENV_DIR}"
...
GIT_LFS_SKIP_SMUDGE=1 "${ENV_DIR}/bin/python" -m pip install "${PIP_INSTALL_ARGS[@]}" -r "${PROJECT_ROOT}/requirements.txt"
...
if not hasattr(cv2, "aruco"):
    raise RuntimeError("OpenCV was installed without the ArUco module")
print("Collection environment import check: OK")
PY

cat <<EOF

Collection environment is ready.
Activate it with:
